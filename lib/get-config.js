...
const {repoUrl} = require('./repository');
...
module.exports = async (context, opts) => {
  ...
  options = {
    ...
    repositoryType: (await pkgRepoType({ normalize: false, cwd })),
    tagFormat: `v\${version}`,
    plugins: [
  ...
  for (const key in process.env)
  {
      ...
      optStr = optStr.replace(new RegExp(envVar, "i"), process.env[key].replace(/\\/, "\\\\"));
  }
...
async function defBranch(opts) {
  const pkg = await readPkgUp(opts);
  return pkg.package && (isPlainObject(pkg.package.repository) ? (pkg.package.repository.type === "git" ? "master" : "trunk") : "master");
}
